...
std::list<std::string> ZSc2ControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZSc2ControlDialog::listMints;

ZSc2ControlDialog::ZSc2ControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZSc2ControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZSc2ControlDialog::~ZSc2ControlDialog()
{
    delete ui;
}

void ZSc2ControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSc2ControlDialog::updateList()
{
...
void ZSc2ControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSc2ControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZSc2_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSc2ControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZSc2ControlDialog::GetSelectedMints()
{
...
void ZSc2ControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
