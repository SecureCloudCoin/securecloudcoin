...
void PrivacyDialog::on_pushButtonZSc2Control_clicked()
{
    ZSc2ControlDialog* zSc2Control = new ZSc2ControlDialog(this);
    zSc2Control->setModel(walletModel);
    zSc2Control->exec();
}

void PrivacyDialog::setZSc2ControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzSc2Selected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzSC2()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid SecureCloudCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    if (!ZSc2ControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZSc2ControlDialog::GetSelectedMints();
    }
    ...
    // Clear zsc2 selector in case it was used
    ZSc2ControlDialog::listSelectedMints.clear();
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelzSC2AmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
